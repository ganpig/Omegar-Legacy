...
class App:
    ...
    def show_button(self, name: str, icon: pygame.Surface, pos: tuple, align: str = 'topleft', todo=easygui.msgbox, background: str = '', text: str = '', text_align: str = 'midright', button_align: str = 'midleft', todo_with_arg: bool = False) -> None:
        """
        添加或更新并显示按钮。
        """
        if name not in self.buttons:
            self.buttons[name] = Button(
                self.window, icon, pos, align, todo, background, text, text_align, button_align, todo_with_arg)
        else:
            self.buttons[name].move(pos)
        return self.buttons[name].show()

    def show_slider(self, name: str, pos: tuple, length: int, width: int, align: str = 'topleft', get_value=None, set_value=None, set_directly=None, get_text=None) -> None:
        """
        添加或更新并显示滑动条。
        """
        if name not in self.sliders:
            self.sliders[name] = Slider(
                self.window, pos, length, width, align, get_value, set_value, set_directly, get_text)
        else:
    ...
    def export_project_wizard(self) -> None:
        ...
        try:
            if easygui.ynbox('是否使用已有的生成设置？（首次使用请选择后者）', '导出工程', ('使用已有生成设置', '创建新的生成设置')):
                using_pickle = True
    ...
    def draw_pick_beats(self, start) -> None:
        ...
        t = self.show_button(
            'play_pause', ICONS['play'], (10, 10), 'topleft', play_pause, 'rect', '播放/暂停', 'midleft', 'midright', True)
    ...
    def draw_settings(self, start) -> None:
        ...
        if self.window.bg_mode == 'image':
            ...
            t = self.show_slider('change_mask_alpha', (SPLIT_LINE+10, t.bottom+15),
                                 WINDOW_SIZE[0]-SPLIT_LINE-20, 10, 'topleft', lambda: self.window.mask_alpha/255, self.window.set_mask_alpha)
