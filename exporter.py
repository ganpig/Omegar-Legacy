...
def convert_charts(charts_info: list) -> None:
    ...
    for chart_info in charts_info:
        chart_info['omgc_path'] = tempfile.mkstemp()[1]
        convert_chart(chart_info['json_path'], chart_info['omgc_path'])
        chart_info['md5'] = hashlib.md5(
            open(chart_info['omgc_path'], 'rb').read()).hexdigest()
...
def export_wizard() -> None:
    ...
    if popup.yesno('是否使用已有的生成设置?', '导出'):
        ...
        while True:
            if chart_projects:
                if not popup.yesno(f'已添加 {len(chart_projects)} 张谱面，是否继续？', '导出'):
                    break
